...
def _require_capability(obj: object, protocol: Any, capability_name: str) -> None:
    if not isinstance(obj, protocol):
        raise TypeError(
            f"{type(obj).__name__} must support {capability_name}."
        )
...
def as_likelihood_model(
    ...
    *,
    ...
    Existing objects satisfying :class:`SupportsLikelihood` are returned
    unchanged. Plain callables are wrapped in :class:`LikelihoodMeasurementModel`
    using the callback convention ``likelihood(measurement, state)``.
    """
...
def sample_next_state(
    model: SupportsTransitionSampling,
    state: Any,
    n: int = 1,
) -> Any:
...
def require_model_attribute(model: object, *names: str) -> Any:
    ...
    This helper is intended for structural model adapters that support canonical
    names and legacy aliases, such as ``system_noise_cov`` and ``sys_noise_cov``.
    """
    ...
    raise AttributeError(
        f"{type(model).__name__} must expose one of the following attributes: "
        f"{options}."
    )
...
def linear_transition_arguments(model: object) -> LinearTransitionArguments:
    ...
    The adapter accepts ``system_noise_cov`` or ``sys_noise_cov`` for the noise
    covariance and ``sys_input`` or ``system_input`` for the optional input.
    """
    ...
    return LinearTransitionArguments(
        ...
        sys_input=get_optional_model_attribute(
            model,
            "sys_input",
            "system_input",
        ),
...
def linear_measurement_arguments(model: object) -> LinearMeasurementArguments:
    ...
    The adapter accepts ``meas_noise`` or ``measurement_noise_cov`` for the
