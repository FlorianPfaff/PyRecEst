...
def test_attribute_helpers_report_missing_or_non_callable_attributes():
    ...
    with pytest.raises(ProtocolAssertionError, match="missing"):
        assert_has_attribute(obj, "missing")
    with pytest.raises(ProtocolAssertionError, match="callable"):
        assert_callable_attribute(obj, "value")
    with pytest.raises(ProtocolAssertionError, match="must not be None"):
...
def test_shape_helpers_report_mismatches():
    ...
    with pytest.raises(ProtocolAssertionError, match="shape"):
        assert_shape(value, (3, 2))
    with pytest.raises(ProtocolAssertionError, match="start"):
        assert_shape_prefix(value, (3,))
    with pytest.raises(ProtocolAssertionError, match="trailing dimension"):
...
def test_common_dimension_helpers_reject_invalid_dimensions():
    ...
    with pytest.raises(ProtocolAssertionError, match="non-negative"):
...
def test_filter_basic_contract_returns_point_estimate():
    estimate = assert_filter_basic_contract(DemoFilter())

    assert_shape(estimate, (2,))
