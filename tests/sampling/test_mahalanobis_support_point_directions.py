...
def test_mahalanobis_support_points_normalize_extreme_finite_direction() -> None:
    mean = np.zeros(2)
    covariance = np.eye(2)
    direction = np.full(2, np.finfo(float).max / 2.0)
    ...
    assert np.isclose(support[0] @ support[0], 1.0)
